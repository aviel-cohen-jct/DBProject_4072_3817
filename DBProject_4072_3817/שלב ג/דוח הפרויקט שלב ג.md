...
**שלב 4 — אימות שאילתות שלב ב':** כל השאילתות מקובץ `שלב ב/Queries.sql` הורצו מחדש על המסד המשולב — **כולן רצות ללא שגיאות**, כמצופה משיטה ב' (אף טבלה קיימת לא שונתה). להלן צילומי הריצה, שאילתה-שאילתה (טבלאות ארוכות קוצרו לתצוגה — מוצגות 6 השורות הראשונות ומספר השורות הכולל):

**הוכחה שהריצה על המסד המשולב** — ‏`fantasy_db`, ‏22 טבלאות, 500 מיפויים ב-`PLAYER_MAPPING`:

![הוכחת מסד משולב](StageB_Proof.png)

**שאילתה 1** — מציאת כל השחקנים שמעולם לא נקנו על ידי אף משתמש (שתי גרסאות — NOT IN / NOT EXISTS). מחזירה 0 שורות — תוצאה נכונה: כל השחקנים נקנו לפחות פעם אחת:

![שאילתה 1](StageB_Query1.png)

**שאילתה 2** — המשתמשים שהוציאו הכי הרבה כסף על עסקאות קנייה (BUY) בשנת 2026 (JOIN מול תת-שאילתה מקושרת — פלט זהה בשתי הגרסאות):

![שאילתה 2](StageB_Query2.png)

**שאילתה 3** — ספירת רשומות היסטוריית המחירים עבור כל שחקן:

![שאילתה 3](StageB_Query3.png)

**שאילתה 4** — משתמשים שמחזיקים בסגל שחקן מקבוצת 'Muxagata' (0 שורות — אין קבוצה כזו בנתונים הנוכחיים; תוצאה עקבית עם שלב ב'):

![שאילתה 4](StageB_Query4.png)

**שאילתה 5** — סיכום נפח עסקאות חודשי תוך פירוק תאריכים (138 שורות, מוצגות הראשונות):

![שאילתה 5](StageB_Query5.png)

**שאילתה 6** — חמשת השחקנים עם תנודתיות המחיר הגבוהה ביותר:

![שאילתה 6](StageB_Query6.png)

**שאילתה 7** — משתמשים ששווי הסגל שלהם גדול מהתקציב הפנוי:

![שאילתה 7](StageB_Query7.png)

**שאילתה 8** — השוואת מחיר שחקן לממוצע בעמדה שלו:

![שאילתה 8](StageB_Query8.png)

**פקודות ה-UPDATE וה-DELETE של שלב ב'** — כולן מתבצעות בהצלחה על המסד המשולב:

![פקודות עדכון ומחיקה](StageB_DML.png)
...
| `StageB_Proof.png`, `StageB_Query1..8.png`, `StageB_DML.png` | צילומי הרצת שאילתות שלב ב' על המסד המשולב (0 שגיאות) |
