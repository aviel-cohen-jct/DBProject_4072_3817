...
> 📄 **הדוח המלא של שלב ג'**, הכולל את אלגוריתם ההינדוס לאחור, החלטות האינטגרציה, וכל השאילתות עם הפלטים: [דוח הפרויקט שלב ג](<DBProject_4072_3817/שלב ג/דוח הפרויקט שלב ג.md>)
...
# פנטזי ליג — דוח פרויקט שלב ד'

<div dir="rtl">

> 📄 **הדוח המלא של שלב ד'**, הכולל תיאור מילולי, קוד מלא, וצילומי מסך אמיתיים מהרצה של כל תוכנית: [דוח הפרויקט שלב ד](<DBProject_4072_3817/שלב ד/דוח הפרויקט שלב ד.md>)

## 1. סקירה — תכנות PL/pgSQL

בשלב ד' נכתבו תוכניות PL/pgSQL על גבי בסיס הנתונים המורחב משלב ג' (בורסת השחקנים + המערכת שהתקבלה + טבלת הגישור `PLAYER_MAPPING`), הכוללות טריגרים, פונקציות, פרוצדורות ובלוקים אנונימיים. כל התוכניות הורצו בפועל מול בסיס הנתונים המקומי והוכחו בצילומי מסך.

## 2. שינוי סכמה — AlterTable.sql

הועשרה טבלת `USERS` בשתי עמודות: `Initial_Budget` (תקציב התחלתי קבוע, ברירת מחדל 100,000) ו-`Portfolio_Yield` (אחוז תשואה, מחושב ומתעדכן ע"י הפונקציה בסעיף 4).

## 3. טריגרים

| טריגר | אירוע | תפקיד |
|---|---|---|
| `trg_players_price_history` | `AFTER UPDATE` על `PLAYERS.Current_Price` | יוצר אוטומטית רשומת `PRICE_HISTORY` בכל שינוי מחיר — הפרדת אחריות בין לוגיקה עסקית לתיעוד |
| `trg_transactions_validate` | `BEFORE INSERT` על `TRANSACTIONS` | הגנה כפולה (Defense in Depth): בודק תקציב מספיק ב-BUY ובעלות בפועל ב-SELL, ברמת בסיס הנתונים — בנוסף לבדיקות הקיימות ב-`server/index.js` |

## 4. פונקציות

| פונקציה | תפקיד | אלמנטים |
|---|---|---|
| `fn_calculate_portfolio_value` | מחשבת שווי סגל כולל ואחוז תשואה למשתמש, מעדכנת `Portfolio_Yield` | Cursor מפורש, רשומה (`t_portfolio_result`), Exception |
| `fn_get_user_transactions` | מחזירה REF CURSOR פתוח על היסטוריית העסקאות של משתמש | REF CURSOR |

## 5. פרוצדורות

| פרוצדורה | תפקיד | אלמנטים |
|---|---|---|
| `prc_process_round_price_update` | מדמה סגירת מחזור — מעדכנת מחירי כל שחקני הבורסה לפי ביצועים אמיתיים (`PLAYERMATCHSTATS`, דרך `PLAYER_MAPPING`) | Cursor מפורש+מרומז, הסתעפויות, לולאה |
| `prc_execute_trade` | מבצעת עסקת קנייה/מכירה שלמה; תופסת חריגה שנזרקת ע"י `trg_transactions_validate` ומדווחת בלי לקרוס | Exception מקונן, DML (INSERT/UPDATE/DELETE) |

## 6. תוכניות ראשיות

שני בלוקים אנונימיים (`DO $$ ... END $$`), כל אחד מזמן פונקציה אחת ופרוצדורה אחת: הראשון מדגים תשואת סגל לפני/אחרי עסקת קנייה, השני מדגים סגירת מחזור ולאחריה מיצוי REF CURSOR של היסטוריית עסקאות.

## 7. קבצי ההגשה — `DBProject_4072_3817/שלב ד/`

| קובץ | תוכן |
|---|---|
| `AlterTable.sql` | שינוי סכמת USERS |
| `Triggers.sql` | 2 הטריגרים |
| `Functions.sql` | 2 הפונקציות |
| `Procedures.sql` | 2 הפרוצדורות |
| `Main.sql` | 2 הבלוקים האנונימיים |
| `backup4.sql` | גיבוי מלא של המסד (22 טבלאות + כל אובייקטי שלב ד) |
| `דוח הפרויקט שלב ד.md` | הדוח המלא, כולל צילומי מסך |

**תג הגשה:** `v4.0.0-phase-d`
...
> 📄 **הדוח המלא של שלב ה'**, הכולל הסבר על הארכיטקטורה, מפת מסכים מלאה, מיפוי דרישות וצילומי מסך מהרצה אמיתית: [דוח הפרויקט שלב ה](<DBProject_4072_3817/שלב ה/דוח הפרויקט שלב ה.md>)
> 🚀 **הוראות כניסה והפעלה:** [הוראות הפעלה](<DBProject_4072_3817/שלב ה/הוראות הפעלה.md>)

## 1. סקירה — ממשק גרפי מלא מעל בסיס הנתונים

בשלב ה' הרחבנו את ממשק ה-React של "ספורטקס" לאפליקציה שלמה שעובדת מול כל 22 הטבלאות של המסד המשולב (7 של הבורסה, 15 של הליגה האמיתית), מפעילה את 8 שאילתות שלב ב' ואת כל 4 תתי-התוכניות של שלב ד', ומראה בפועל את השפעת הטריגרים. המערכת בנויה כאפליקציה אמיתית עם שני תפקידים:

| תפקיד | מסכים | מה עושים |
|---|---|---|
| **סוחר** | שוק השחקנים, פרופיל שחקן, הסגל שלי, היסטוריית פעולות, תובנות שוק, החשבון שלי | קנייה/מכירה דרך `prc_execute_trade`, שווי ותשואה מ-`fn_calculate_portfolio_value`, היסטוריה דרך REF CURSOR, דוחות שלב ב' עם פילטרים, עריכת החשבון |
| **מנהל ליגה** | מחזורים, שחקני בורסה, משתמשים, יומן עסקאות, קבוצות ואצטדיונים, שחקנים ושוערים, מאמנים ושופטים, משחקים | CRUD מלא לכל טבלה (הוספה / עדכון לפי מפתח / מחיקה לפי מפתח), מסכי master-detail, סגירת מחזור עם `prc_process_round_price_update` |

## 2. כלים ודרך העבודה

- **Frontend:** React 19 + TypeScript + Vite + Tailwind CSS (המשך המסכים משלב א').
- **Backend:** Node.js + Express + `pg`. שרת API דק עם SQL פרמטרי בלבד.
- **בסיס נתונים:** PostgreSQL 18 ב-Docker, משוחזר מ-`backup4.sql` של שלב ד' בפקודה `npm run db:restore`.
- **ארכיטקטורה מונחית מטא-דאטה:** קובץ אחד (`server/meta.js`) מתאר את 22 הטבלאות (מפתחות, מפתחות זרים, סוגי שדות, ביטוי "שם תצוגה"). ממנו נבנים אוטומטית ה-API הגנרי (`server/crud.js`) והטפסים בממשק (`src/components/EntityPanel.tsx`), כך שכל טבלה מקבלת CRUD מלא ללא קוד ייעודי.
- **שמות במקום מזהים:** לכל מפתח זר השרת מבצע `LEFT JOIN` ומחזיר את שם היעד; בטפסים בוחרים לפי שם דרך חיפוש.
- **עדכון לפי מפתח:** המשתמש מזין מפתח → המערכת טוענת את הרשומה → עורכים ושומרים.
...
| | |
|---|---|
| ![כניסה](images/stageE_01_login.png) | ![פרופיל שחקן – קנייה דרך הפרוצדורה](images/stageE_04_profile_buy.png) |
...
| קובץ | תוכן |
|---|---|
| `הוראות שלב ה.md` | ההוראות המקוריות של השלב |
| `הוראות הפעלה.md` | הוראות כניסה והפעלה של המערכת |
| `דוח הפרויקט שלב ה.md` | הדוח המלא עם צילומי מסך |
| `src/`, `server/`, `db/` (בשורש הריפו) | קוד הממשק, השרת וסקריפט השחזור |
